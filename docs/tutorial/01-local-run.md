...
This tutorial gets the local stack running with Aspire (`AppHost + API + Qdrant`) and verifies health + OpenAPI.
...
- `/health` returns `200`
- `/ready` returns `200` when API can reach Qdrant
- OpenAPI JSON is reachable

## Troubleshooting

`/ready` returns `503` with `Qdrant endpoint is not configured...` or `Qdrant is unreachable.`:
